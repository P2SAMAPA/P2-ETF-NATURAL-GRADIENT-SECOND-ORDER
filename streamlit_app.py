...
st.set_page_config(page_title="Natural Gradient Second-Order (K-FAC / Shampoo)", layout="wide")
...
st.markdown('<h1 style="text-align: center;">🧠 Natural Gradient Second‑Order Engine</h1>', unsafe_allow_html=True)
st.markdown('<p style="text-align: center;">K‑FAC / Shampoo – Kronecker‑factored approximate curvature for deep portfolio networks</p>', unsafe_allow_html=True)

st.sidebar.markdown("## 🧬 Second-Order Natural Gradient")
...
st.sidebar.markdown(f"**Optimizer:** {config.OPTIMIZER.upper()}")
st.sidebar.markdown(f"**Hidden layers:** {config.HIDDEN_SIZES}")
...
def find_latest_json(files):
    json_files = [f for f in files if f.endswith('.json') and 'ng_second_order_' in f]
    if not json_files:
        return None
...
def display_universe(universe_name, uni_data, window_data, window_label):
    ...
    for idx, etf in enumerate(top3):
        with cols[idx]:
            ...
            <div class="hero-card">
                <h3>{etf['ticker']}</h3>
                <p>Natural grad score: {etf['ng_score_norm']:.3f}</p>
...
with tab1:
    st.header("🧠 Top ETFs by Natural Gradient Score (Auto Best Window)")
    with st.expander("📖 Interpretation", expanded=False):
        st.markdown("""
        - **Second‑order natural gradient** methods (K‑FAC, Shampoo) precondition the gradient with an approximation of the Fisher information matrix.
        - This leads to much faster convergence than standard SGD or first‑order natural gradient.
        - The network learns a ranking function from historical features to next‑day returns.
        - The score = predicted return (regression output). Higher score → stronger long signal.
...
with tab2:
    ...
    st.markdown("Choose a rolling window to inspect the natural gradient scores per ETF.")
    for universe_name, uni_data in data["universes"].items():
        if not uni_data or not uni_data.get("all_windows"):
            st.warning(f"No window data for {universe_name}")
            continue
...
st.sidebar.markdown("---")
st.sidebar.caption("Natural Gradient Second‑Order | K‑FAC / Shampoo for deep portfolio networks")
